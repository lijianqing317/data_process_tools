...
def select_same_name_file(template_path,templateformat,soure_path,target_path,targetformat=[],operate_copy_or_move_flag = True):
    for file_name in os.listdir(template_path):
        ...
        if file_name.endswith(templateformat):
            for i in targetformat:
                try:
                    target_name = file_name.replace(templateformat,i)
                    print('target_name122', target_name)
...
def args_para1():
    ...
    parser.add_argument("--targetformat",default=['json'])
...
def args_para():
    ...
    parser.add_argument("--data", type=str,default='data_no_topk')
    parser.add_argument("--data_select",type=str,default='data_no_topk')
    parser.add_argument("--data_no_select",type=str,default='data_no_topk_jsons')
    args = parser.parse_args()
    return args
...
if __name__ == '__main__':
    ...
    for train_val_file in os.listdir(data_select):
        ...
        for data_batch_name in os.listdir(sub_train_val_file):
            ...
            select_same_name_file(template_path,'json',source_path,target_path,targetformat=['json'],operate_copy_or_move_flag = True)

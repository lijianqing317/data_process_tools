...
class CutLabelme(object):
    ...
    def cut_json(self,json_p):
        ...
        for index_object in range(len(cut_one_img)):
            ...
            new_name_img = '{}_{}_{}_{}.jpg'.format(mid_point[index_object][0],mid_point[index_object][1],index_object,image_name_path)
            new_name_json = '{}_{}_{}_{}.json'.format(mid_point[index_object][0],mid_point[index_object][1],index_object,image_name_path)
...
def get_parser():
    ...
    parser.add_argument('--workspace', default='/media/lijq/f373fb19-ec6a-4a1c-96e5-3f2013f3f5c6/byd/workspace/workspace_0315',
                        type=str, help='', )
    parser.add_argument('--data_dir', default='data_cropped',
                        type=str, help='', )
    parser.add_argument('--save_dir', default='data_cropped768',
                        type=str, help='', )
...
if __name__ == '__main__':
    ...
    # for data in os.listdir(args.workspace):
    data_path = os.path.join(args.workspace, args.data_dir)
    save_data_path = os.path.join(args.workspace, args.save_dir)
    for train_val in os.listdir(data_path):
        train_val_path = os.path.join(data_path, train_val)
        save_train_val_path=os.path.join(save_data_path,train_val)
        for labelme_data in os.listdir(train_val_path):
            try:
                labelme_data_path = os.path.join(train_val_path, labelme_data)
                save_labelme_data_path = os.path.join(save_train_val_path, labelme_data)
                print('save_labelme_data_path:', save_labelme_data_path)
                jsons_path_list = labelme_data_path
                if not os.path.exists(save_labelme_data_path):
                    print('save_labelme_data_path:ing', save_labelme_data_path)
                    os.makedirs(save_labelme_data_path)
                else:
                    print('oooo')

                CutLabelme(labelme_data_path,save_labelme_data_path,args.crop_size_w,args.crop_size_h,args.process_nums,args.padding,args.line_width)
            except:
                print('----------------')